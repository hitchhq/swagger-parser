 ...
 */
function validateSpec(api, errors) {
  ...
  paths.forEach(function(pathName) {
    ...
    if (path && pathName.indexOf('/') === 0) {
      validatePath(api, path, pathId, errors);
    }
  });
  ...
  swaggerMethods.forEach(function(operationName) {
    ...
    if (operation) {
      ...
      responses.forEach(function(responseName) {
        ...
        validateResponse(responseName, response, responseId, errors);
      });
    }
  });
}
 ...
 */
function validateParameters(api, path, pathId, operation, operationId, errors) {
  var pathParams = path.parameters || [];
  var operationParams = operation.parameters || [];

  // Check for duplicate path parameters
  checkForDuplicates(pathParams, errors, pathId);

  // Check for duplicate operation parameters
  checkForDuplicates(operationParams, errors, operationId);
  ...
  validateBodyParameters(params, operationId, errors);
  validatePathParameters(params, pathId, operationId, errors);
  validateParameterTypes(params, api, operation, operationId, errors);
}
 ...
 * @param   {string}    operationId  -  A value that uniquely identifies the operation
 */
function validateBodyParameters(params, operationId, errors) {
  ...
  if (bodyParams.length > 1) {
    errors.push({
      path: `${operationId}/parameters`,
      message: util.formatMessage('%s has %d body parameters. Only one is allowed.',
 ...
 * @param   {string}    operationId   - A value that uniquely identifies the operation
 */
function validatePathParameters(params, pathId, operationId, errors) {
    ...
    for (var j = i + 1; j < placeholders.length; j++) {
      if (placeholders[i] === placeholders[j]) {
        errors.push({
          path: `${operationId}/parameters`,
          message: util.formatMessage(
          '%s has multiple path placeholders named %s', operationId, placeholders[i])
        });
      }
    }
  }
  ...
  params
    ...
    .forEach(function(param) {
      if (param.required !== true) {
        errors.push({
          path: `${operationId}/parameters/${param.name}`,
          message: util.formatMessage(
      ...
      if (match === -1) {
        errors.push({
          path: `${operationId}/${param.name}`,
          message: util.formatMessage(
            '%s has a path parameter named "%s", ' +
  ...
  if (placeholders.length > 0) {
    errors.push({
      path: `${operationId}/parameters`,
      message: util.formatMessage('%s is missing path parameter(s) for %s', operationId, placeholders)
    });
  }
}
 ...
 * @param   {string}    operationId  -  A value that uniquely identifies the operation
 */
function validateParameterTypes(params, api, operation, operationId, errors) {
  params.forEach(function(param, index) {
    var parameterId = operationId + '/parameters/' + index;
    ...
    validateSchema(schema, parameterId, validTypes, errors);
    ...
    if (schema.type === 'file') {
      ...
      if (consumes.indexOf('multipart/form-data') === -1 &&
        ...
        errors.push({
          path: `${operationId}/parameters`,
          message: util.formatMessage(
            '%s has a file parameter, so it must consume multipart/form-data ' +
 ...
 * @param   {object[]}  params  - An array of Parameter objects
 */
function checkForDuplicates(params, errors, path) {
    ...
    for (var j = i + 1; j < params.length; j++) {
      ...
      if (outer.name === inner.name && outer.in === inner.in) {
        errors.push({
          ...
          message: util.formatMessage('Found multiple %s parameters named "%s"', outer.in, outer.name)
        });
      }
    }
  }
 ...
 */
function validateResponse(code, response, responseId, errors) {
  if (code !== 'default' && (code < 100 || code > 599)) {
    errors.push(util.formatMessage('%s has an invalid response code (%s)', responseId, code));
  }
  ...
  if (response.schema) {
    ...
    if (validTypes.indexOf(response.schema.type) === -1) {
      errors.push({
        path: responseId,
        message: util.formatMessage(
          '%s has an invalid response schema type (%s)', responseId, response.schema.type)
      });
    }
  }
}
 ...
 */
function validateSchema(schema, schemaId, validTypes, errors) {
  if (validTypes.indexOf(schema.type) === -1) {
    errors.push({
      path: schemaId,
      message: util.formatMessage(
        '%s has an invalid type (%s)', schemaId, schema.type)
    });
  }
  ...
  if (schema.type === 'array' && !schema.items) {
    errors.push({
      path: schemaId,
      message: util.formatMessage('%s is an array, so it must include an "items" schema', schemaId)
    });
  }
}
